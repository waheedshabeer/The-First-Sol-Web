...
export const MyExperties = ({ userData }) => {
  ...
  return (
    ...
    <div className="mt-20 text-left overflow-hidden" id="Service">
      <h1 data-aos="fade-right" className="headings text-white">My Experties</h1>
        ...
        {/* first grid column */}
        {
          ExpertiesList.length > 0 && ExpertiesList.map((item, index) => {
            return <div className={`p-2 sm:p-5 md:p-8 ${(index + 1) % 2 === 0 ? "border-b" : "border-b border-r "} border-#EFEFEF`}>
              ...
              <h1 className="text-lg sm:text-xl md:text-2xl pt-4 font-semibold text-white">{item.Category}</h1>
              ...
              <div
                ...
                className="font-medium text-aqua-normal pt-3 sm:pt-6 text-xs sm:text-lg  cursor-pointer  transform duration-700 hover:scale-x-105">
